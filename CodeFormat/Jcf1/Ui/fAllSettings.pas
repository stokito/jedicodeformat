...
type
  ...
  private
    ...
    function GetFrameType(const psName: string): TSettingsFrameClass;
...
uses frFiles, frObfuscateSettings,
  frClarify, frClarifySpaces, frClarifyIndent,
  frClarifyReturns, frClarifyBlocks, frClarifyAlign,
...
var
  lcNode: TTreeNode;
begin
  { the ide pluggin doesn't have the about box so show it here }

  {$IFDEF IDEPLUGGIN}

    // basic settings page at the start'
    lcNode := tvFrames.Items.GetFirstNode;
    tvFrames.Items.Insert(lcNode, 'Basic');

    //about page at the end
    tvFrames.Items.Add(nil, 'About');
  {$ENDIF}
